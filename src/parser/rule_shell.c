...
enum parser_status parse_else_clause(struct ast **ast, struct lexer *lexer){
    ...
    else if (lexer_peek(lexer).type == TOKEN_ELIF){ // check elif token
        lexer_pop(lexer);
        *ast = (struct ast *)ast_if_init();

        if (parse_compound_list(&(((struct ast_if*)ast)->condition), lexer) != PARSER_OK) // check compound list
            return PARSER_UNEXPECTED_TOKEN;
        ...
        if (parse_compound_list(&(((struct ast_if*)ast)->then_body), lexer) != PARSER_OK) // check compound list
            return PARSER_UNEXPECTED_TOKEN;

        if (parse_else_clause(&(((struct ast_if*)ast)->else_body), lexer) != PARSER_OK) // check else_clause
            return PARSER_UNEXPECTED_TOKEN;
        return PARSER_OK;
    }
    else
