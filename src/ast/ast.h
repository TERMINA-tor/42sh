...
struct ast_loop *ast_loop_condition_add(enum ast_type type, struct ast *ast, struct ast *condition);    // add condition to loop node, if ast not found init the an if loop and add the condition
struct ast *ast_loop_body_add(enum ast_type type, struct ast *ast, struct ast *body);   // add body to loop, if ast not found return NULL
